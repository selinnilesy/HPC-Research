...
int readCSRFormat(int z, double ratio) {
    ...
    vector<double> tempVec;
        const fs::path matrixFolder{"/home/selin/Split-Data/" + matrix_names[z] +"/inner/CSR-Data"};
        for(auto const& dir_entry: fs::directory_iterator{matrixFolder}){
            std::fstream myfile(dir_entry.path(), std::ios_base::in);
           if(dir_entry.path().stem() == to_string(ratio) + "-row") {
                int tempValInt;
           ...
           else if(dir_entry.path().stem() == to_string(ratio) +"-col") {
                int tempValInt;
                vector<int> tempVecInt;
                while (myfile >> tempValInt) {
                    tempVecInt.push_back(tempValInt);
...
int main(int argc, char **argv){
    ...
    readCSRFormat(atoi(argv[1]), atof(argv[2]));
    ...
    myfile1.open ("/home/selin/Split-Data/" + matrix_names[inputType] + "/inner/CSR-Data/rowptr.txt", ios::out | ios::trunc);
    myfile2.open ("/home/selin/Split-Data/" + matrix_names[inputType] + "/inner/CSR-Data/colind.txt", ios::out | ios::trunc);
    myfile3.open ("/home/selin/Split-Data/" + matrix_names[inputType] + "/inner/CSR-Data/vals.txt", ios::out | ios::trunc);
    myfile4.open ("/home/selin/Split-Data/" + matrix_names[inputType] + "/inner/CSR-Data/diag.txt", ios::out | ios::trunc);
