...
int main(int argc, char **argv) {
    ...
    if(my_rank==0) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            //if(matrixRowDiff[i] == 0 ) cout << "matrixRowDiff is 0 for row " << i << endl;
        }
    }
        ...
        for (int i = 0; i < myPieceSize; i++) {
            for (int j = accum_colInd; j < accum_colInd + myRowDiff[i]; j++) {
                ...
                if (colInd < my_rank * pieceSize) {
                    if (lastConfSquare != colInd / pieceSize &&
                        find(confSquares.begin(), confSquares.end(), colInd / pieceSize) == confSquares.end()) {
    ...
    //for (int i = 0; i < confSquares.size(); i++) cout << "Rank: " << my_rank << " confs in squares: " << confSquares[i] << endl << flush;
    ...
    else if(my_rank && my_rank < world_size-1 &&  my_rank < firstEmptyProcess-1){
    ...
    else if(my_rank < firstEmptyProcess){
        MPI_Recv(&neighbourSize, 1, MPI_INT, my_rank-1, 0, MPI_COMM_WORLD, &status);
        neighbourX = new double[neighbourSize];
        MPI_Recv(neighbourX, neighbourSize, MPI_INT, my_rank-1, 0, MPI_COMM_WORLD, &status);
    }
    //if(my_rank==61) cout << "61 arrives." << endl;
    ...
    if(my_rank && my_rank < firstEmptyProcess){
    ...
    //if(my_rank==61) cout << "Rank: " << my_rank << " completed exchaning Xs." << endl;
    ...
    int colIndModulo;
    //if(my_rank==61) cout << "Rank: " << my_rank << " starting MPI_Wtime " << endl;
    ...
    MPI_Win_fence(0, window);
    cout << "Rank: " << my_rank << " completed MPI_Accumulate " << endl;
    ...
    cout << "Rank: " << my_rank << " completed MPI_Gatherv " << endl;
    ...
    // process outer region sequentially : only in root.
